...
public class RouteRepository extends BaseRepository {
    ...
    public List<Route> getRoutesForMichal() {
        ...
        Route route = new Route(1, 3.5, 215, R.drawable.route_1, R.drawable.route_1, "Traska dla emerytów", "To jest własnie taka jakaś lajtowa traska, bo pomimo tego, że jej przejście to poad dwie godziny, to ogarnia niewielki zakres Arboretum. Polecamy ją zatem osobom starszym, które mają już za słabe stawy i kolana do długiego zwiedzania.");
        routes.add(route);
        route = new Route(2, 1.5, 315, R.drawable.route_2, R.drawable.route_2, "Dla rodzin z dziećmi", "Traska przechodzi przez wiele terenów zakrzaczonych, gdzie wasze małe urwisy będą się wam mogły zgubić w lesie i będziecie mieli święty spokój chociaż przez chwilę, by móc podziwiać zbiory roślin w naszym Arboretum.");
        routes.add(route);
        route = new Route(3, 7.2, 420, R.drawable.route_3, R.drawable.route_3, "Trasa idealna dla poszukiwaczy przygód", "W sumie nie mam pomysłu na opis, ale jakoś tak mi ci poszukiwacze przygód wpadli, jak dałam jako obrazek ikonkę mapki.... :D");
        routes.add(route);
        route = new Route(4, 5., 200, R.drawable.route_4, R.drawable.route_4, "Trasa rododendronowa", "W końcu czy nie po to każdy przyjeżdża do Arboretum? Z powodu sławnych rododendronów?\n(teraz się zastanawiam czy zrobi się ta spacja czy nie)\nNIEEEE - my tam przyjedziemy jedynie służbowo :(");
        routes.add(route);
